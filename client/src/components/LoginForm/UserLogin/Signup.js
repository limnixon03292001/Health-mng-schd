...
const Signup = ({ setisSwitch }) => {
  return (
    <div className='w-full h-full'>
        <h3 className="my-4 text-gray-800 text-2xl text-left font-semibold">
            Register your Account.
        </h3>
        <div className="relative z-0 w-full mb-6 group">
            <input type="email" name="floating_email" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none  focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
        ...
        <div className="mb-6 ">
            <button type="submit" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium text-sm w-full sm:w-auto px-5 py-2.5 text-center rounded-full">Register</button>
        </div>
        <div>
            <hr className="mb-4 border-t" />
        </div>
        <div className="text-center">
            <span className="cursor-pointer inline-block text-sm text-blue-500 align-baseline hover:text-blue-800" onClick={() => setisSwitch(false)}>
